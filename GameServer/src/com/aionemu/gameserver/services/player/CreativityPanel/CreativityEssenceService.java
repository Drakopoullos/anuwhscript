...
public class CreativityEssenceService
{
    ...
    PlayerCPDAO cpDAO = DAOManager.getDAO(PlayerCPDAO.class);
    private int point;
        ...
        if (player.isArchDaeva()) {
			...
			for (PlayerCPEntry ce : player.getCP().getAllCP()) {
				if (ce.getSlot() <= 6 && ce.getPoint() >= 255) {
					...
					PacketSendUtility.sendPacket(player, new SM_PLAYER_ESSENCE_CONTROL(0, size, ce.getSlot(), ce.getPoint()));
				} if (ce.getSlot() >= 15 && ce.getSlot() <= 400 || ce.getSlot() >= 409 && ce.getSlot() <= 456) {
					PacketSendUtility.sendPacket(player, new SM_PLAYER_ESSENCE_CONTROL(0, size, ce.getSlot(), ce.getPoint()));
				}
			}
			PacketSendUtility.sendPacket(player, new SM_PLAYER_ESSENCE(totalPoint, player.getCPStep(), size, true));
		}
	}
		...
		if (player.isArchDaeva() && player.getCreativityPoint() < CustomConfig.CP_LIMIT_MAX) {
			if (percent >= 16.66f && percent < 33.33f) {
				...
				if (step == 2) {
					...
					int totalPoint = player.getCreativityPoint();
					int size = DAOManager.getDAO(PlayerCPDAO.class).getSlotSize(player.getObjectId());
					PacketSendUtility.sendPacket(player, new SM_PLAYER_ESSENCE(totalPoint, 3, size, false));
					PacketSendUtility.sendPacket(player, new SM_STATS_INFO(player));
					//You have gained Essence.
					//Click the Essence icon displayed on the Character XP meter, SHIFT+U, or select Start Menu.
					//Add Essence to open the Allocate Essence window.Open Allocate Essence.
				...
				if (step == 3) {
					...
					int totalPoint = player.getCreativityPoint();
					int size = DAOManager.getDAO(PlayerCPDAO.class).getSlotSize(player.getObjectId());
					PacketSendUtility.sendPacket(player, new SM_PLAYER_ESSENCE(totalPoint, 4, size, false));
					PacketSendUtility.sendPacket(player, new SM_STATS_INFO(player));
					//You have gained Essence.
					//Click the Essence icon displayed on the Character XP meter, SHIFT+U, or select Start Menu.
					//Add Essence to open the Allocate Essence window.Open Allocate Essence.
				...
				if (step == 4) {
					...
					int totalPoint = player.getCreativityPoint();
					int size = DAOManager.getDAO(PlayerCPDAO.class).getSlotSize(player.getObjectId());
					PacketSendUtility.sendPacket(player, new SM_PLAYER_ESSENCE(totalPoint, 5, size, false));
					PacketSendUtility.sendPacket(player, new SM_STATS_INFO(player));
					//You have gained Essence.
					//Click the Essence icon displayed on the Character XP meter, SHIFT+U, or select Start Menu.
					//Add Essence to open the Allocate Essence window.Open Allocate Essence.
				...
				if (step == 5) {
					...
					int totalPoint = player.getCreativityPoint();
					int size = DAOManager.getDAO(PlayerCPDAO.class).getSlotSize(player.getObjectId());
					PacketSendUtility.sendPacket(player, new SM_PLAYER_ESSENCE(totalPoint, 6, size, false));
					PacketSendUtility.sendPacket(player, new SM_STATS_INFO(player));
					//You have gained Essence.
					//Click the Essence icon displayed on the Character XP meter, SHIFT+U, or select Start Menu.
					//Add Essence to open the Allocate Essence window.Open Allocate Essence.
	 ...
	 * - The amount of Creativity a "ArchDaeva" can acquire with each level has been increased.
	 * - If you have already leveled up the difference will be paid the next time you reach the Creativity point.
		...
		if (player.isArchDaeva()) {
			...
			for (PlayerCPEntry ce : player.getCP().getAllCP()) {
				if (ce.getSlot() >= 7 && ce.getSlot() <= 14) {
					vesselCount++;
					PacketSendUtility.sendPacket(player, new SM_PLAYER_ESSENCE_CONTROL(0, vesselCount, ce.getSlot(), ce.getPoint()));
				} else if (ce.getSlot() >= 401 && ce.getSlot() <= 408) {
					avatarCount++;
					PacketSendUtility.sendPacket(player, new SM_PLAYER_ESSENCE_CONTROL(0, avatarCount, ce.getSlot(), ce.getPoint()));
				}
			}
	...
	public void onResetEssence(Player player, int plusSize) {
		...
		PacketSendUtility.sendPacket(player, new SM_STATS_INFO(player));
		PacketSendUtility.sendPacket(player, new SM_PLAYER_ESSENCE(player.getCreativityPoint(), player.getCPStep(), 0, false));
	}
