...
public class DecomposeAction extends AbstractItemAction
{
	...
	public boolean canAct(Player player, Item parentItem, Item targetItem) {
		...
		if (decomposeLevel == -1 || decomposeLevel > player.getLevel()) {
			...
			SelectItems selectitem = DataManager.DECOMPOSABLE_SELECT_ITEM_DATA.getSelectItem(player.getPlayerClass(), parentItem.getItemId());
			if (selectitem == null) {
	...
	public void act(final Player player, final Item parentItem, final Item targetItem) {
		...
		if (this.isSelect) {
			...
			player.getController().addTask(TaskId.ITEM_USE, ThreadPoolManager.getInstance().schedule(new Runnable() {
				@Override
				public void run() {
					SelectItems selectitems = DataManager.DECOMPOSABLE_SELECT_ITEM_DATA.getSelectItem(player.getPlayerClass(), parentItem.getItemId());
					if (selectitems == null) {
						PacketSendUtility.sendPacket(player, SM_SYSTEM_MESSAGE.STR_DECOMPOSE_ITEM_INVALID_STANCE(parentItem.getNameId()));
						return;
					} else {
					...
					if (selectedCollection.getItems().size() > 0) {
						...
						for (RandomItem randomItem : selectedCollection.getRandomItems()) {
							...
							if (randomType != null) {
								...
								case MANASTONE_COMMON_GRADE_65:
...
//				PacketSendUtility.broadcastPacketAndReceive(player, new SM_ITEM_USAGE_ANIMATION(player.getObjectId(), parentItem.getObjectId(), parentItem.getItemId(), 0, validAction ? 1 : , 0));
				PacketSendUtility.broadcastPacketAndReceive(player, new SM_ITEM_USAGE_ANIMATION(player.getObjectId(), parentItem.getObjectId(), parentItem.getItemId(), 0, 1, 1));
				player.getController().cancelTask(TaskId.ITEM_USE);
			}
