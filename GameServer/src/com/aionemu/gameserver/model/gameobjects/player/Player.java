...
import com.aionemu.gameserver.model.team2.alliance.*;
...
public class Player extends Creature
{
	...
	public boolean isEnemyFrom(Player enemy) {
		if (this.getObjectId() == enemy.getObjectId()) {
			...
			return true;
		} else if (enemy.getBattleground() != null && this.getBattleground() != null) {
			return true;
		} else if (enemy.isBandit() && this.isBandit()) {
			return true;
		} else if (FFAService.getInstance().isInArena(enemy) && enemy.isFFA()) {
			return true;
		} else if (!enemy.getRace().equals(getRace()) || getController().isDueling(enemy) || enemy.isBandit()) {
			return true;
		} else {
			return false;
		}
